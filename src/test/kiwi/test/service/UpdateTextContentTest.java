...
//            			.get("1"), "");
	...
	/**
	 * This tests a concrete bug as experienced by Steffi 
	 * (styling of diffs when kiwi links are involved)
	 * @throws Exception
	 */
	@Test(dependsOnMethods = { "checkInit" })
	public void testBug0710a() throws Exception {
    	...
    	new FacesRequest() {
            ...
            protected void invokeApplication() {
            	...
            	Document xom1 = UpdateTextContentSupport.getDocumentFromFile("xml_texts/bug_0710a.xml");
    	...
    	new FacesRequest() {
            ...
            protected void invokeApplication() {
            	...
            	Document xom2 = UpdateTextContentSupport.getDocumentFromFile("xml_texts/bug_0710b.xml");
    	...
    	new FacesRequest() {
            ...
            protected void invokeApplication() {
            	...
            	// The TextContentUpdate should have 1 different node: /div[1]/p[1]
            	if(diffs.size() != 1) {
            		log.error(">>>>>>>>>>>>>>>>> diff size is #0 ", diffs.size());
            	} else {
            		log.info("blaaaaaaaaaaaaa");
            	}
            	Assert.assertEquals(diffs.size(), 1);
            	...
            	for(int i=0; i<1; i++) {
            		String xpath = diffs.get(i).getXPath();
            		if(xpath.equals("/*[local-name()='div'][1]/*[local-name()='p'][1]")) {
            			if(!diffs.get(i).getChangedWordsMap().containsKey("000001.000001")) {
            				log.error("diff does not contain 000001.000001");
            				log.error(diffs.get(i).getChangedWords());
            			}
            			if(!diffs.get(i).getChangedWordsMap().containsKey("000002.999999")) {
            				log.error("diff does not contain 000002.999999");
            				log.error(diffs.get(i).getChangedWords());
            			}
            			log.info("everything ok");
            		} else {
            			log.info("diff returned wrong xpath: #0 ", xpath);
            			Assert.fail("diff returned wrong xpath: " + xpath);
            		}
            	}
            	UpdateTextContentService updateTextContentService = 
            		(UpdateTextContentService) Component.getInstance("updateTextContentService");
            	try {
					Document preview = updateTextContentService.setStyleWithXOM(
							currentContentItem.getTextContent().getXmlDocument(), diffs, 
							currentContentItem.getVersions()
								.get(currentContentItem.getVersions().size()-1).getRevision().getId(), 
								true);
					log.info("#0 ", preview.toXML());
				} catch (Exception e) {
					e.printStackTrace();
				}
            }
	 ...
	 */
	@Test(dependsOnMethods = { "checkInit" })
	public void testBug0710b() throws Exception {
    	...
    	new FacesRequest() {
            ...
            protected void invokeApplication() {
            	...
            	Document xom1 = UpdateTextContentSupport.getDocumentFromFile("xml_texts/bug_0710b.xml");
    	...
    	new FacesRequest() {
            ...
            protected void invokeApplication() {
            	...
            	Document xom2 = UpdateTextContentSupport.getDocumentFromFile("xml_texts/bug_0710a.xml");
    	...
    	new FacesRequest() {
            ...
            protected void invokeApplication() {
            	...
            	// The TextContentUpdate should have 1 different node: /div[1]/p[1]/a[1]
            	if(diffs.size() != 1) {
            		log.error(">>>>>>>>>>>>>>>>> diff size is #0 ", diffs.size());
            	} else {
            		log.info("blaaaaaaaaaaaaa1");
            	}
            	Assert.assertEquals(diffs.size(), 1);
            	
            	for(int i=0; i<1; i++) {
            		String xpath = diffs.get(i).getXPath();
            		if(xpath.equals("/*[local-name()='div'][1]/*[local-name()='p'][1]/*[local-name()='a'][1]")) {
            			if(!diffs.get(i).getChangedWordsMap().containsKey("999999.000000")) {
            				log.error("diff does not contain 999999.000000");
            				log.error(diffs.get(i).getChangedWords());
            			}
            			if(!diffs.get(i).getChangedWordsMap().containsKey("000000.000000")) {
            				log.error("diff does not contain 000000.000000");
            				log.error(diffs.get(i).getChangedWords());
            			}
            			log.info("everything ok");
            		} else {
