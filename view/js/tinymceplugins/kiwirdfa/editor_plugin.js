(function() {
	tinymce.create('tinymce.plugins.KiWiRDRaPlugin', {
		...
		init : function(ed, url) {
			...
			kiwirdfaJSLib._setAboutProperties = function (t, div, uri, relation, types) {
				
				if (div != null) {
					// We are wrapping a newly creted div, create the appropriate control elements
					t.editingAbout = t.rdfa.createAboutFromAboutNode(div);
				}
				
				if (t.editingAbout != null) {
					t.rdfa.setAboutUri (t.editingAbout, uri);
					t.rdfa.setAboutRelation (t.editingAbout, relation);
					t.rdfa.setAboutTypes (t.editingAbout, types);
				}
				t.editingAbout = null;
			}.partial(t);
			...
			kiwirdfaJSLib._getContext = function (t, element) {
				var contextElement = t.rdfa.getLeastRDFaAboutElement (element);
				if (contextElement != null) {
					return t.rdfa.getAboutUri(contextElement);
				}
				return null;
			}.partial(t);
			
			kiwirdfaJSLib._getContextTypes = function (t, element) {
				var contextElement = t.rdfa.getLeastRDFaAboutElement (element);
				if (contextElement != null) {
					return t.rdfa.getAboutTypes(contextElement);
				}
				return null;
			}.partial(t);
			
			kiwirdfaJSLib._setLinkProperties = function (t, span, resource, rel, property, content, text) {
				...
				if (t.editingLink != null) {
					...
					t.rdfa.setLink(t.editingLink, resource, rel, property, content, text);
...
/*
			...
			ed.addCommand('mceKiWiRDFaAbout', function(t, ed) {
				...
				if (t.selectedAboutElement != null) {
					context = t.rdfa.getAboutUri(t.selectedAboutElement);
					contextTypes = t.rdfa.getAboutTypes(t.selectedAboutElement);
				}
				
				kiwirdfaJSLib.showKiwirdfaAboutPanel (context, contextTypes, null, null, null, null);
			...
			ed.addCommand('mceKiWiRDFaLink', function(t, ed) {
				...
				if (t.selectedLinkElement == null) {
					...
					kiwirdfaJSLib.showKiwirdfaLinkPanel (context, contextTypes, null, null, null, null, null, t.rdfa.getLinkText(span));
				}
				else {
					t.editingLink = t.selectedLinkElement;
					t.creating = false;
					
					kiwirdfaJSLib.showKiwirdfaLinkPanel (
							context, 
							contextTypes,
							null,
							t.rdfa.getLinkResource(t.editingLink), 
							t.rdfa.getLinkRel(t.editingLink), 
							t.rdfa.getLinkProperty(t.editingLink), 
							t.rdfa.getLinkContent(t.editingLink),
							t.rdfa.getLinkText(t.editingLink));
			...
			*/
			ed.onInit.add( function(t, ed) {
				...
				t.rdfa.onAboutModify = function (t, div) {
					...
					if (contextElement != null) {
						...
						contextTypes = t.rdfa.getAboutTypes(contextElement);
					}
					
					kiwirdfaJSLib.showKiwirdfaAboutPanel (context, contextTypes, null, t.rdfa.getAboutUri(div), t.rdfa.getAboutRelation(div), t.rdfa.getAboutTypes(div));
				}.partial(t);
			...
			ed.onNodeChange.add ( function(t, ed, cm, e) {
				...
				var prevSelectedLinkElement = t.selectedLinkElement;
				t.selectedLinkElement = t.rdfa.getLeastRDFaLinkElement (e);
				
				if (prevSelectedLinkElement != t.selectedLinkElement) {
					cm.setActive('kiwirdfaLink', t.selectedLinkElement != null);
				}
